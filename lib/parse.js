...
function parse(psdPath) {
  return PSD.open(psdPath).then(function (psd) {
    var linkNodes = getLinkNodes(psd.tree());
    var hotspots = parseLinks(linkNodes);
    ...
    setImageDimensions(psd, state, breakpointName);
...
function createAssets(assets) {
  var screenJSON = createConfig(assets);

  return Promise.map(assets, saveImage)
    .then(function() {
