...
function setImageDimensions(group, state, breakpointName) {
  state.breakpoints[breakpointName].imageWidth = group.layer.header.width;
  state.breakpoints[breakpointName].imageHeight = group.layer.header.height;
}

function parseLink(node, rootNode) {
  var imageWidth = node.layer.header.width;
  var imageHeight = node.layer.header.height;
